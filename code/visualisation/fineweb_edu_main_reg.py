...
for edu, registers in edu_dictionary.items():
    ...
    for register, value in registers.items():
        ...
        final_dict[row_counter] = [edu, register_name, percent, label]
        row_counter +=1 
...
for row, value in final_dict.items():
    educ = value[0]
    bar_label = f"{educ} \n \n {edu_percentages[educ]} %"
    final_dict[row].append(bar_label)
df = pd.DataFrame.from_dict(final_dict, orient="index", columns=["edu","main_register","percentage", "text", "bar_label"])


fig = px.bar(df, x="bar_label", y="percentage", color="main_register", barmode="stack", text="text")

fig.update_layout(font_size = 14,
                    legend_title_text='Main register', 
                      yaxis_title_text="Percent", xaxis_title_text="Propella-4b educational value, percent of total dataset",
