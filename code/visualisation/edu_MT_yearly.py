...
with open ("C:/Users/shkunn/Downloads/crawl_year.txt", "r") as f:
    ...
    for crawl, content in file.items():
        ...
        for edu, value in content.items():
            MT_sum = 0
            for register, total in value["register"].items():
                if "MT" in register:
                    MT_sum += total
            year_dict[year][edu] += MT_sum
...
for year, content in year_dict.items():
    ...
    for edu, num in content.items():
        dict_for_df[row_counter] = [year,edu, (num/total) *100]
        row_counter += 1
...
bar.update_layout(legend_title_text='Educational value', 
        title_text="Yearly distribution of educational values in MT texts (inc. hybrids)",
                      yaxis_title_text="Percent", xaxis_title_text="Year of Common Crawl")
