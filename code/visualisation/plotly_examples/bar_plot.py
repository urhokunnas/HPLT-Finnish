...
#i find this is the simplest way to ensure the years are in the right order
#
    ...
    for crawl, content in file.items():
        ...
        for edu, item in content.items():
            for register, value in item["register"].items():
                if targeted_register in register: #this preserves registers
                    if register in year_dict[year].keys():
                        year_dict[year][register] += value
                    else:
                        year_dict[year][register] = value
                else:                       #for stuff outside the targeted register we just care about the year and sum
